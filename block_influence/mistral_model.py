        ...
        for idx, decoder_layer in enumerate(self.layers):
            if self.selected_blocks is not None:  # skip blocks that are not selected
                if idx not in self.selected_blocks:
                    if use_cache:
                        if next_decoder_cache is None:  # initialization for the key-value
                            next_decoder_cache = past_key_values
                        dummy_tensor = torch.empty((0, 0, 0),)  # empty tensor for concatenate inside cache to work (BDL format)
                        next_decoder_cache.update(dummy_tensor, dummy_tensor, idx, None)  # following the original mistral format
                    if output_attentions:
                        all_self_attns += (None,)
                    continue
